...
发布复验：`a84ebd2` 前端已更新 13511，33 个 HTTP 资源一致。手机明暗两组直接加载线上产物，彩色圆点与标签、中性外框、终态区分和减少动态效果均通过；原进程、后端、配置及 7 个验收会话保留，无重启、无备份。截图与具体测试地址见 `output/playwright/active-status-colors/verification.md`。
