...
- 切换测试实例前先核对 systemd PID、监听端口与 cwd；确认 runtime-info 的 `busy: false`、无活动回合、pending 请求及队列为空。备份原 unit、runner 和构建产物。切换后同时验证 HTTP、实际 CLI 握手、运行目录、模型目录和保留的验收线程。
- 源码回退不是运行时回退。正式实例没有升级，无需正式回滚。测试回滚应指向保存的旧 WebUI 产物，同时保留隔离 CODEX_HOME 和完整固定 CLI，不直接恢复会共享正式 HOME 的原 runner。
- 今后正式升级必须单独确认维护窗口，先备份原 home/config/catalog，再按测试通过的发布产物和固定 CLI 切换；不要让不同 CLI 进程共用同一数据库做升级验收。
