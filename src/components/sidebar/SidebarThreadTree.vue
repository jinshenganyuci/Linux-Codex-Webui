  ...
  <section class="thread-tree-root" :class="{ 'chats-first': showChatsFirst }">
    <section v-if="pinnedThreads.length > 0" class="pinned-section">
      ...
      >
        ...
        <span class="thread-tree-header">{{ t('Pinned chats') }}</span>
      </SidebarMenuRow>
    ...
    <p v-if="pinnedThreadActionError" class="thread-tree-action-error" role="alert">
    ...
    <Teleport to="body">
      ...
      >
        ...
        <button
          ...
          @click="onTogglePinFromMenu(openThreadMenuThread.id)"
        >
          {{ isPinned(openThreadMenuThread.id) ? t('Unpin this chat') : t('Pin this chat') }}
        </button>
...
const isSavingPinnedThreadState = ref(false)
const pinnedThreadActionError = ref('')
...
watch([pinnedThreadIds, threadById, () => props.isLoading], () => {
  if (!hasLoadedPinnedThreadState.value) return
...
function describeAutomationSchedule(rrule: string): string {
  ...
  if (frequency === 'MINUTELY') return `RRULE: ${rrule} · ${t(interval === 1 ? 'runs every {count} minute' : 'runs every {count} minutes', { count: interval })}`
  if (frequency === 'HOURLY') return `RRULE: ${rrule} · ${t(interval === 1 ? 'runs every {count} hour' : 'runs every {count} hours', { count: interval })}`
  if (frequency === 'DAILY' && parts.INTERVAL !== undefined) return `RRULE: ${rrule} · ${t(interval === 1 ? 'runs every {count} day' : 'runs every {count} days', { count: interval })}`
