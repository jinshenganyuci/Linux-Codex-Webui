<template>
  <section v-if="state.threadId" class="native-thread-controls" data-testid="native-thread-controls" aria-label="原生会话控制">
    ...
    <div v-if="state.expanded" class="native-controls-body">
      <section aria-label="原生运行设置">
        <h3>模型与运行设置</h3>
      ...
      <section v-if="state.capabilities.permissions" aria-label="原生会话权限">
        <h3>会话权限</h3>
      ...
      <section v-if="state.capabilities.goals" aria-label="原生 Goal">
        <h3>原生 Goal <span v-if="state.goal" class="native-controls-muted">· {{ goalStatus }}</span></h3>
      ...
      <section v-if="state.capabilities.queue || state.mode === 'native'" aria-label="队列归属">
...
import { nativeSubmissionText, type NativeSettingsPatch, type NativeSubmission } from '../../nativeThreadControls'
...
watch(() => state.value.threadId, () => { goalDirty.value = false; confirmClear.value = false; editingId.value = ''; visibleQueueCount.value = 20; objective.value = ''; budget.value = '' })
