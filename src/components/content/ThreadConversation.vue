                  ...
                  <div class="file-change-panel-inner">
                    ...
                    <div v-if="isFileChangeActionable(readStandaloneFileChangeSummary(message))" class="file-change-actions">
                      ...
                      <button
                        ...
                        :title="fileChangeNextAction(readStandaloneFileChangeSummary(message)) === 'redo' ? t('Redo file changes from this turn') : t('Undo file changes from this turn')"
                        :aria-label="fileChangeNextAction(readStandaloneFileChangeSummary(message)) === 'redo' ? t('Redo file changes from this turn') : t('Undo file changes from this turn')"
                  ...
                  <div class="file-change-panel-inner">
                    ...
                    <div v-if="isFileChangeActionable(readAnchoredFileChangeSummary(message))" class="file-change-actions">
                      ...
                      <button
                        ...
                        :title="fileChangeNextAction(readAnchoredFileChangeSummary(message)) === 'redo' ? t('Redo file changes from this turn') : t('Undo file changes from this turn')"
                        :aria-label="fileChangeNextAction(readAnchoredFileChangeSummary(message)) === 'redo' ? t('Redo file changes from this turn') : t('Undo file changes from this turn')"
...
import {
  ...
  parseInlineSegments,
  resolveRelativePath,
} from './thread-conversation/markdownInline'
import { parseMessageBlocks } from './thread-conversation/markdownBlocks'
import {
  escapeHtml,
...
} from './thread-conversation/markdownHtml'
import type {
  InlineSegment,
  ListItem,
  MessageBlock,
} from './thread-conversation/markdownTypes'
...
function commandGroupSummaryLabel(message: UiMessage): string {
  ...
  return `${countLabel} · ${t('latest: {command}', { command: latestCommand })}`
}
...
const markdownInlineContext = { toLocalThreadUrl }
...
function formatFileChangeCountLabel(count: number): string {
  return t(count === 1 ? '1 file changed' : '{count} files changed', { count })
}
...
function inferDiffViewerLanguage(change: UiFileChange): string {
  ...
  return resolveCodeLanguageAlias(extension)
}
...
function getInlineSegments(text: string): InlineSegment[] {
  ...
  return setBoundedCacheEntry(
    inlineSegmentCache,
    text,
    parseInlineSegments(text, markdownInlineContext),
    INLINE_SEGMENT_CACHE_LIMIT,
  )
}
...
const inlineHtmlContext = { toBrowseUrl }

function renderInlineSegmentsAsHtml(text: string): string {
  return renderInlineSegmentsToHtml(getInlineSegments(text), inlineHtmlContext)
}
...
function renderListItemContentAsHtml(item: ListItem): string {
  return renderListItemContentToHtml(item, messageBlockHtmlContext)
}

function renderMessageBlockAsHtml(block: MessageBlock): string {
  return renderMessageBlockToHtml(block, messageBlockHtmlContext)
}
  ...
  emit('respondServerRequest', {
    ...
    result: {
      ...
      contentItems: [
        {
          type: 'inputText',
          text: 'Tool call rejected from Linux-Codex-Webui UI.',
        },
      ],
    },
  })
...
function onRejectUnknownRequest(requestId: number): void {
  emit('respondServerRequest', {
    ...
    error: {
      code: -32000,
      message: 'Rejected from Linux-Codex-Webui UI.',
    },
  })
}
