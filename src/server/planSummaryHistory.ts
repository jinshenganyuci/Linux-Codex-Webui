...
import type { UiPlanSummary, UiPlanSummaryHistoryState } from '../types/codex.js'

const STATE_VERSION = 1
const STATE_FILE_NAME = 'linux-codex-webui-plan-summary-history.json'
const LOCK_RETRY_MS = 25
const LOCK_TIMEOUT_MS = 10_000
const LOCK_STALE_MS = 30_000

type StoredPlanSummaryHistoryState = {
  version: typeof STATE_VERSION
  threads: UiPlanSummaryHistoryState
}
...
export function getPlanSummaryHistoryPath(): string {
  const configuredHome = process.env.CODEX_HOME?.trim()
  const codexHome = configuredHome || join(homedir(), '.codex')
  return join(codexHome, STATE_FILE_NAME)
}

async function readStoredState(statePath: string): Promise<StoredPlanSummaryHistoryState> {
  try {
    const raw = await readFile(statePath, 'utf8')
    return {
      version: STATE_VERSION,
      threads: normalizePlanSummaryHistoryState(JSON.parse(raw) as unknown),
    }
  } catch (error) {
...
async function writeStoredStateAtomic(
  ...
  const payload: StoredPlanSummaryHistoryState = {
    version: STATE_VERSION,
    threads: normalizePlanSummaryHistoryState(threads),
  }
  try {
...
async function acquireFileLock(statePath: string): Promise<() => Promise<void>> {
  ...
  while (true) {
    ...
    if (Date.now() - startedAt >= LOCK_TIMEOUT_MS) {
      throw new Error('Timed out waiting for the plan summary history lock')
    }
    await new Promise<void>((resolve) => setTimeout(resolve, LOCK_RETRY_MS))
  }
}

async function mutatePlanSummaryHistory<T>(
  update: (state: UiPlanSummaryHistoryState) => { state: UiPlanSummaryHistoryState; result: T },
): Promise<T> {
  const run = mutationChain.then(async () => {
    const statePath = getPlanSummaryHistoryPath()
...
export async function readPlanSummaryHistory(threadIdInput?: unknown): Promise<UiPlanSummaryHistoryState> {
  await mutationChain.catch(() => {})
  const threads = (await readStoredState(getPlanSummaryHistoryPath())).threads
...
export async function writePlanSummary(input: unknown): Promise<UiPlanSummary> {
  const summary = normalizePlanSummary(input)
  if (!summary) throw new Error('Invalid plan summary')

  return await mutatePlanSummaryHistory((current) => {
    const next = {
      ...current,
      [summary.threadId]: upsertPlanSummary(current[summary.threadId] ?? [], summary),
    }
    return {
      state: normalizePlanSummaryHistoryState(next),
      result: summary,
    }
  })
}

export async function deletePlanSummaryHistory(threadIdInput: unknown): Promise<void> {
  const threadId = normalizeThreadId(threadIdInput)
  if (!threadId) throw new Error('Missing threadId')
  await mutatePlanSummaryHistory((current) => {
...
export async function listPlanSummaryHistoryRecoveryFiles(): Promise<string[]> {
  const statePath = getPlanSummaryHistoryPath()
